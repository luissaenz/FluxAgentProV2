...
def main():
    ...
    transcript_timeline_content = read_file(
        "components/transcripts/TranscriptTimeline.tsx"
    )
    ...
    check(
        "TranscriptTimeline.tsx existe",
        file_exists("components/transcripts/TranscriptTimeline.tsx"),
    )
    check(
        "Acepta props taskId y orgId",
        "taskId: string" in hook_content and "orgId: string" in hook_content,
    )
    check(
        "El hook useTranscriptTimeline exporta la funcion principal",
    ...
    check(
        "Hook usa useQuery para fetch del snapshot",
        "useQuery" in hook_content and "/transcripts/" in hook_content,
    )
    check(
        "El snapshot se almacena en estado de eventos",
    ...
    check(
        "Hook ordena eventos por sequence",
        "sort" in hook_content
        and "sequence" in hook_content
        and "a.sequence - b.sequence" in hook_content,
    )

    # ── Criterio 4: Suscripcion Realtime recibe eventos nuevos sin recarga ──
    print("\n--- Criterio 4: Suscripcion Realtime ---")
    check("Hook crea canal de Supabase", "supabase.channel" in hook_content)
    check(
        "Suscripcion a postgres_changes INSERT",
        "postgres_changes" in hook_content and "INSERT" in hook_content,
    )
    check("Filtro por aggregate_id", "aggregate_id=eq" in hook_content)
    check("Canal se limpia al desmontar", "removeChannel" in hook_content)
    check(
        "Canal usa nombre dedicado transcript-timeline",
    ...
    check(
        "Hook expone isRunning desde snapshot",
        "isRunning" in hook_content and "snapshot?.is_running" in hook_content,
    )
    check(
        "Componente muestra badge 'En vivo' cuando isRunning",
        "En vivo" in transcript_timeline_content
        and "isRunning" in transcript_timeline_content,
    )
    check(
        "No muestra En vivo si la task esta en estado terminal",
    ...
    check(
        "Componente tiene logica de auto-scroll con ref",
        "autoScrollRef" in transcript_timeline_content,
    )
    check(
        "Detecta scroll manual hacia arriba",
        "distanceToBottom" in transcript_timeline_content
        or "scrollTop" in transcript_timeline_content,
    )
    check(
        "Boton 'Ir al final' aparece cuando usuario scrollea arriba",
        "Ir al final" in transcript_timeline_content
        or "ScrollButton" in transcript_timeline_content,
    )
    check(
        "Auto-scroll se activa al recibir eventos nuevos",
    ...
    check(
        "Hook expone connectionStatus",
        "connectionStatus" in hook_content and "ConnectionStatus" in hook_content,
    )
    check(
        "Hook expone funcion reconnect",
        "reconnect" in hook_content and "export" in hook_content,
    )
    check(
        "Reintento automatico con timeout",
        "RETRY_INTERVAL_MS" in hook_content or "setTimeout" in hook_content,
    )
    check(
        "UI muestra estado de error de conexion",
        "Error de conexion" in transcript_timeline_content
        or "Desconectado" in transcript_timeline_content,
    )
    check("Boton de reintentar en UI", "Reintentar" in transcript_timeline_content)

    # ── Criterio 10: Estado vacio con mensaje apropiado ──
    print("\n--- Criterio 10: Estado vacio ---")
    check(
        "Muestra mensaje cuando no hay eventos y esta running",
        "Esperando eventos" in transcript_timeline_content,
    )
    check(
        "Muestra mensaje cuando no hay eventos y no esta running",
    ...
    check(
        "TimelineEvent renderiza CodeBlock para tool_output",
        "CodeBlock" in timeline_event_content
        and "tool_output" in timeline_event_content,
    )
    check(
        "CodeBlock se renderiza solo si payload tiene contenido",
        "Object.keys(event.payload).length > 0" in timeline_event_content
        or "event.payload &&" in timeline_event_content,
    )

    # ── Criterio 12: Cleanup del canal Supabase al desmontar ──
    print("\n--- Criterio 12: Cleanup del canal ---")
    check(
        "Hook removeChannel en cleanup del useEffect",
        "removeChannel" in hook_content and "return ()" in hook_content,
    )

    # ── Criterio 13: Aislamiento multi-tenant ---
    print("\n--- Criterio 13: Aislamiento multi-tenant ---")
    check("Hook requiere orgId como parametro", "orgId: string" in hook_content)
    check(
        "Filtro de suscripcion usa taskId (que ya tiene aislamiento por org en backend)",
        "aggregate_id=eq" in hook_content,
    )
    check(
        "Query de snapshot usa endpoint REST que incluye org header (api.get)",
    ...
    check(
        "No hay TODOs en los archivos creados",
        "// TODO" not in hook_content
        and "// TODO" not in timeline_event_content
        and "// TODO" not in transcript_timeline_content,
    )
    check(
        "No hay imports no usados evidentes (verificacion basica)",
        "useEffect" in hook_content
        and "useState" in hook_content
        and "useCallback" in hook_content
        and "useRef" in hook_content,
    )
    check(
        "La pagina de transcript importa TranscriptTimeline",
        "TranscriptTimeline" in transcript_page_content,
    )
    check(
        "La pagina de transcript ya no usa useFlowTranscript directamente",
    ...
    print(
        f"Resultado: {PASSED}/{total} passed, {FAILED}/{total} failed, {WARNINGS}/{total} warnings"
    )
