...
class TestRequestApproval:
    ...
    @pytest.mark.asyncio
    async def test_creates_pending_approval_row(
        ...
        await flow.request_approval(
            description="Aprobar operación", payload={"monto": 100_000}
        )
    ...
    @pytest.mark.asyncio
    async def test_updates_state_to_awaiting(
        self, mock_service_client, mock_tenant_client, sample_org_id
    ):
    ...
    @pytest.mark.asyncio
    async def test_stores_approval_payload(
        self, mock_service_client, mock_tenant_client, sample_org_id
    ):
...
class TestResume:
    ...
    @pytest.mark.asyncio
    async def test_resume_approved_calls_on_approved(
        self, mock_service_client, sample_org_id
    ):
        ...
        with patch.object(
            flow, "_on_approved", new_callable=AsyncMock
        ) as mock_approved:
            await flow.resume(
                task_id="task_test", decision="approved", decided_by="supervisor1"
            )
            mock_approved.assert_called_once()

    @pytest.mark.asyncio
    async def test_resume_rejected_calls_on_rejected(
        self, mock_service_client, sample_org_id
    ):
        ...
        with patch.object(
            flow, "_on_rejected", new_callable=AsyncMock
        ) as mock_rejected:
            await flow.resume(
                task_id="task_test", decision="rejected", decided_by="supervisor1"
            )
            mock_rejected.assert_called_once_with("supervisor1")

    @pytest.mark.asyncio
    async def test_resume_stores_decision_in_state(
        self, mock_service_client, sample_org_id
    ):
    ...
    @pytest.mark.asyncio
    async def test_resume_raises_when_no_snapshot(
        self, mock_service_client, sample_org_id
    ):
