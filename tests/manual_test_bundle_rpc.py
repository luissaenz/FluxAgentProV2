...
async def run_rpc_test():
    ...
    payload = BundleRPCPayload(
        ...
        skills={
            "atomic_skill.py": "def test(): return True"
        }
    )
    ...
    try:
        with get_tenant_client(org_id) as db:
            response = db.rpc("import_bundle_atomic", {
                "p_org_id": org_id,
    ...
    try:
        with get_tenant_client(org_id) as db:
            db.rpc("import_bundle_atomic", {
                "p_org_id": org_id,
                "p_payload": bad_payload
            }).execute()
            logger.error("❌ ERROR: El RPC no falló como se esperaba.")
    except Exception as e:
        logger.info(f"✅ ÉXITO: El RPC falló correctamente (Rollback esperado). Error: {str(e)[:50]}...")
