...
def _load_inputs(input_str: Optional[str], input_file: Optional[Path]) -> Dict[str, Any]:
...
def run_skill(
    ...
    try:
        ...
        if result is not None:
            ...
            console.print("[yellow]Skill finished with no 'result' variable set.[/yellow]")
...
async def _run_local_agent(role: str, bundle_path: Path, inputs: Dict[str, Any]):
    ...
    console.print(f"[cyan]Loading local bundle from [bold]{bundle_path}[/bold]...[/cyan]")
...
async def _poll_task(client: httpx.AsyncClient, api_url: str, task_id: str, headers: dict, timeout: int):
