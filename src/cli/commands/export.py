...
def export_agents(
    ...
    output: Path = typer.Option(Path("migration_bundle"), "--output", "-o", help="Directory to save the exported bundle"),
):
    ...
    try:
        ...
        if not result.data:
            print(f"[yellow]![/yellow] No agents found for organization [bold]{org_id}[/bold].")
        ...
        print(f"  2. Run [cyan]fap package {output}[/cyan] to create the migration ZIP.")
