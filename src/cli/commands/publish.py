...
def publish_bundle(
    ...
    force: bool = typer.Option(
        False, "--force", "-f", help="Force overwrite existing bundle version"
    ),
):
    ...
    print(
        f"Publishing [bold]{zip_path.name}[/bold] to [bold]{config.api_url}[/bold]..."
    )
        ...
        with open(zip_path, "rb") as f:
            ...
            with httpx.Client(base_url=config.api_url, timeout=60.0) as client:
                response = client.post(
                    "/api/bundles/import", headers=headers, files=files, params=params
                )
                ...
                if response.status_code == 201:
                    ...
                    print(
                        "[red]Error:[/red] Authentication failed (401). Try logging in again."
                    )
                else:
