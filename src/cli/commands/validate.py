    ...
    try:
        ...
        with httpx.Client(base_url=config.api_url, timeout=5.0) as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"[yellow]⚠️  Could not sync security config (Status {response.status_code}). Using local defaults.[/yellow]"
                )
    except Exception as e:
        print(
            f"[yellow]⚠️  Offline or connection error: {e}. Using local defaults.[/yellow]"
        )
...
def validate_bundle(
    ...
    sync: bool = typer.Option(
        False, "--sync", "-s", help="Synchronize security configuration with server"
    ),
):
    ...
    if sync:
        ...
        if remote_config:
            ...
            if remote_py and remote_py != local_py:
                print(
                    f"[yellow]⚠️  Python version mismatch! Server: {remote_py}, Local: {local_py}[/yellow]"
                )
                print(
                    "[yellow]   Bytecode compilation might behave differently.[/yellow]"
                )
            else:
