...
from .bundle_schemas import BundleManifest, BundleContent, BundleInfo
...
class BundleManager:
    """Manages the lifecycle of a bundle from ZIP bytes to parsed content."""
    ...
    def process_zip(self, zip_bytes: bytes) -> BundleContent:
        ...
        if size > MAX_ZIP_SIZE:
            raise BundleError(f"Bundle size ({size} bytes) exceeds limit of {MAX_ZIP_SIZE}")
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(zip_bytes)) as z:
                ...
                for rel_path, expected_hash in manifest.hashes.items():
                    if rel_path not in z.namelist():
                        raise BundleError(f"File '{rel_path}' declared in manifest not found in ZIP")
            ...
            raise BundleError(f"JSON parsing error: {str(e)}")
        except Exception as e:
            if isinstance(e, BundleError):
                raise
            logger.exception("Unexpected error processing bundle")
            raise BundleError(f"Internal error processing bundle: {str(e)}")
    ...
    def _parse_file_content(self, path: str, data: bytes, content: BundleContent):
        ...
        if path.startswith("agents/") and path.endswith(".json"):
            ...
            content.skills[filename] = data.decode("utf-8")
        elif path == "manifest.json":
            pass # Already handled
        else:
            logger.info("Ignoring file outside of standard structure: %s", path)
    ...
    def _validate_limits(self, content: BundleContent):
        ...
        if len(content.agents) > MAX_AGENTS:
            raise BundleError(f"Exceeded max agents: {len(content.agents)} > {MAX_AGENTS}")
        ...
        if len(content.skills) > MAX_SKILLS:
            raise BundleError(f"Exceeded max skills: {len(content.skills)} > {MAX_SKILLS}")
