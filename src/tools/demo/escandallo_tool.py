...
class EscandalloInput(BaseModel):
    ...
    duracion_horas: int = Field(
        ..., ge=1, le=24, description="Duración del evento en horas"
    )
    tipo_menu: str = Field(..., description="basico | estandar | premium")
    provincia: str = Field(..., description="Tucuman | Salta | Jujuy | Catamarca")
    factor_climatico_pct: int = Field(
        ..., ge=0, description="Factor climático del Agente 2 (%)"
    )
...
class EscandalloOutput(BaseModel):
    evento_id: str
    # Bloques
    bloque1_productos: int = Field(
        ..., description="Bebidas + hielo + garnish + desechables"
    )
    bloque2_equipamiento: int = Field(..., description="Amortización de equipamiento")
    bloque3_personal: int = Field(
        ..., description="Bartenders + head + asistente × horas"
    )
...
class EscandalloTool(BaseTool):
    ...
    def _calcular_bloque_productos(
        ...
        bebidas_alc = sum(ml_total * mix[cat] * precio_por_ml[cat] for cat in mix)

        bebidas_no_alc = (
            pax * float(consumo["agua_litros_por_persona"]) * PRECIO_AGUA_POR_LITRO
        )

        hielo = pax * float(consumo["hielo_kg_por_persona"]) * PRECIO_HIELO_POR_KG
