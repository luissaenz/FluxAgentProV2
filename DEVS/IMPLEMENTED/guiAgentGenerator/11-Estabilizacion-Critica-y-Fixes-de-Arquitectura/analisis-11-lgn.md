...
|---|---|---|---|---|
| 1 | `templates_seed.py` idempotencia | `src/cli/commands/templates_seed.py:183-193` | ⚠️ Riesgo | Check `SELECT` + `INSERT` sin `ON CONFLICT` |
| 2 | `BuilderBreadcrumb` sync tabs | `dashboard/components/builder/BuilderBreadcrumb.tsx:18` | ⚠️ Discrepancia | Prop `activeTab` recibida pero no actualizada desde Layout |
| 3 | `page.tsx` breadcrumb hardcoded | `dashboard/app/(app)/builder/page.tsx:9` | ❌ Confirmado | `activeTab="agent-form"` hardcodeado |
| 4 | `BuilderLayout` tabs state | `dashboard/components/builder/BuilderLayout.tsx:56` | ✅ Verificado | `useState('agent-form')` existe |
| 5 | Test mock injection | `tests/e2e/test_builder_scenarios.py:176-186` | ⚠️ Discrepancia | `verify_org_membership` override correcto pero otros mocks fallan |
| 6 | `zodResolver` en `AgentForm.tsx` | `dashboard/components/builder/AgentForm.tsx:82` | ✅ Verificado | Uso correcto de `@hookform/resolvers/zod` |
| 7 | `conftest.py` fixtures | `tests/conftest.py:111-141` | ⚠️ Discrepancia | Fixtures no usados por tests de builder (no importan `mock_service_client`) |
| 8 | `agent_templates` tabla RLS | `supabase/migrations/030_agent_templates.sql:25-29` | ✅ Verificado | Politica `authenticated` read, `service_role` write |
...
```sql
-- supabase/migrations/030_agent_templates.sql
...
|---|---|---|---|---|---|---|---|---|---|
| 0 | DX & Tooling | `src/cli/commands/validate_builder_fixes.py` | `def run(check: str) -> bool` | `src/cli/commands/test_builder.py` | DX | Media | 0.5h | Ninguna | → verificar: `uv run fap validate-builder-fixes --help` ejecuta sin errores |
